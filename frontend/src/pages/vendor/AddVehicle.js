...
const AddVehicle = () => {
  ...
  const uploadFileHandler = async (e) => {
    ...
    try {
      ...
      if (e.target.name === 'image') {
        setImage(data.files.image.location);
        setFormData((prev) => ({ ...prev, image: data.files.image.location }));
      } else {
        const imageLocations = data.files.images.map((file) => file.location);
        setImages(imageLocations);
        setFormData((prev) => ({ ...prev, images: imageLocations }));
      }
      setUploading(false);
    } catch (error) {
      console.error(error);
